...
export default {
  ...
  methods: {
    ...
    getAllCriteria() {
      try {
        this.$axios.get("/criteriaAll").then((response) => {
          ...
          this.criteriaList = allItems.slice(4, 5);
    ...
    inputData() {
      ...
      localStorage.setItem("inputData2", JSON.stringify(nilaiData));
      router.push("/check");
    },
  },
  ...
  created() {
    ...
    this.requestData.team = this.selectedTeam;
  },
};
</script>
<style></style>
